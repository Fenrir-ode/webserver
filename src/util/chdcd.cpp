...
-------------------------------------------------*/
std::error_condition file_read(FILE *file, void *buffer, std::uint64_t offset, std::uint32_t length, std::uint32_t &actual)
{
	...
	// perform the read
	std::size_t const count = std::fread(buffer, 1, length, file);
	if ((count < length) && std::ferror(file))
	{
		std::clearerr(file);
		return std::error_condition(errno, std::generic_category());
	}
...
static uint32_t parse_wav_sample(const char *filename, uint32_t *dataoffs)
{
	...
	/* get the total size */
	file_read(file, &filesize, offset, 4, actual);
	offset += actual;
	if (offset < 8)
	{
	...
	filesize = __bswap_32(filesize);

	/* read the RIFF file type and make sure it's a WAVE file */
	file_read(file, buf, offset, 4, actual);
	offset += actual;
	if (offset < 12)
	{
	...
	while (true)
	{
		file_read(file, buf, offset, 4, actual);
		offset += actual;
		file_read(file, &length, offset, 4, actual);
		offset += actual;
	...
	file_read(file, &temp16, offset, 2, actual);
	offset += actual;
	temp16 = __bswap_16(temp16);
	if (temp16 != 1)
	{
	...
	/* sample rate */
	file_read(file, &rate, offset, 4, actual);
	offset += actual;
	rate = __bswap_32(rate);
	if (rate != 44100)
	{
	...
	file_read(file, buf, offset, 6, actual);
	offset += actual;

	/* bits/sample */
	file_read(file, &bits, offset, 2, actual);
	offset += actual;
	bits = __bswap_16(bits);
	if (bits != 16)
	{
...
extern "C" uint32_t mame_parse_toc(const char *tocfname, cdrom_toc_t *out_cdrom_toc, raw_toc_dto_t *fenrir_toc)
{
	...
	{
		// convert...
		out_cdrom_toc->flags = outtoc.flags;
	...
	*/
		...
		for (int i = 0; i < outtoc.numtrks; i++)
		{
			outtoc.tracks[i].logframeofs = 0;

			if (outtoc.tracks[i].pgdatasize == 0)
			{
				logofs += outtoc.tracks[i].pregap;
			}
			else
			{
				outtoc.tracks[i].logframeofs = outtoc.tracks[i].pregap;
			}

			outtoc.tracks[i].physframeofs = physofs;
			outtoc.tracks[i].chdframeofs = 0;
			outtoc.tracks[i].logframeofs += logofs;
			outtoc.tracks[i].logframes = outtoc.tracks[i].frames - outtoc.tracks[i].pregap;

			// postgap adds to the track length
			logofs += outtoc.tracks[i].postgap;

			physofs += outtoc.tracks[i].frames;
			logofs += outtoc.tracks[i].frames;
		}

		// convert to fernir format
		for (int i = 0; i < out_cdrom_toc->numtrks; i++)
		{
			// outtoc
			out_cdrom_toc->tracks[i].chdframeofs = outtoc.tracks[i].chdframeofs;
			out_cdrom_toc->tracks[i].datasize = outtoc.tracks[i].datasize;
			out_cdrom_toc->tracks[i].extraframes = outtoc.tracks[i].extraframes;
			out_cdrom_toc->tracks[i].frames = outtoc.tracks[i].frames;
			out_cdrom_toc->tracks[i].logframeofs = outtoc.tracks[i].logframeofs;
			out_cdrom_toc->tracks[i].logframes = outtoc.tracks[i].logframes;
			out_cdrom_toc->tracks[i].multicuearea = outtoc.tracks[i].multicuearea;
			out_cdrom_toc->tracks[i].padframes = outtoc.tracks[i].padframes;
			out_cdrom_toc->tracks[i].pgdatasize = outtoc.tracks[i].pgdatasize;
			out_cdrom_toc->tracks[i].pgsub = outtoc.tracks[i].pgsub;
			out_cdrom_toc->tracks[i].pgsubsize = outtoc.tracks[i].pgsubsize;
			out_cdrom_toc->tracks[i].pgtype = outtoc.tracks[i].pgtype;
			out_cdrom_toc->tracks[i].physframeofs = outtoc.tracks[i].physframeofs;
			out_cdrom_toc->tracks[i].postgap = outtoc.tracks[i].postgap;
			out_cdrom_toc->tracks[i].pregap = outtoc.tracks[i].pregap;
			out_cdrom_toc->tracks[i].splitframes = outtoc.tracks[i].splitframes;
			out_cdrom_toc->tracks[i].subsize = outtoc.tracks[i].subsize;
			out_cdrom_toc->tracks[i].subtype = outtoc.tracks[i].subtype;
			out_cdrom_toc->tracks[i].trktype = outtoc.tracks[i].trktype;
			// outinfo
			strcpy(out_cdrom_toc->tracks[i].filename, outinfo.track[i].fname.c_str());
			out_cdrom_toc->tracks[i].idx0offs = outinfo.track[i].idx0offs;
